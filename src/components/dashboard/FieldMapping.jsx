...
//         newPolygons.push(polygonData);
//       }

//       if (!newPolygons.length) return;

//       const lastPolygon = newPolygons[newPolygons.length - 1];
//       setAreaInfo(lastPolygon);
//       onAreaData?.(lastPolygon);
//       localStorage.setItem("mapAreaInfo", JSON.stringify(newPolygons));
//     };
...
//               <label
//                 htmlFor="kml-upload"
//                 className="fixed top-32 right-8 flex justify-center items-center gap-2 bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-4 cursor-pointer py-3 rounded-lg shadow-lg z-50 hover:from-emerald-700 hover:to-teal-700 transition-all duration-300"
//               >
//                 <Upload className="w-5 h-5" />
//                 Upload KML
//               </label>
...
// /* MapDetail component remains the same as in your original code */
...
// // import "leaflet-geosearch/dist/geosearch.css";
// // import SearchControl from "../dashboard/SearchControl";
// // import KmlFileUploding from "../dashboard/KmlFileUploding"
...
// //   function getPolygonMeasurements(coords) {
// //     // coords = [[lat,lng],[lat,lng],...]

// //     const turfPolygon = turf.polygon([[...coords.map(c => [c[1], c[0]])]]);
// //     // Leaflet uses [lat,lng]  → Turf needs [lng,lat]

// //     const areaSqMeters = turf.area(turfPolygon); // m²
...
// //       // Extract all <coordinates> nodes (for multiple polygons)
// //       const coordsNodes = xmlDoc.getElementsByTagName("coordinates");
// //       if (!coordsNodes.length) return;
...
// //         if (coordsArray.length < 3) continue; // Skip invalid polygons

// //         // Compute area in km² using your existing function
// //         const areaKm2 = computePolygonAreaKm2(coordsArray);
...
// //       // Save the last polygon as current areaInfo (like drawn shapes)
// //       const lastPolygon = newPolygons[newPolygons.length - 1];
// //       setAreaInfo(lastPolygon);
// //       onAreaData?.(lastPolygon);

// //       // Save all polygons in localStorage
// //       localStorage.setItem("mapAreaInfo", JSON.stringify(newPolygons));

// //       // Update state for map drawing
// //       setPolygons(newPolygons.map((p) => p.coordinates));
// //     };

// //     reader.readAsText(file);
// //   }

// //     // KMl file Functionality End
// //   return (
// //     <ProtectedPage>
// //       <div className="flex w-full  overflow-hidden bg-gray-50">
// //         <div className="flex-1 flex flex-col overflow-hidden">
// //           <div className="flex-1 overflow-y-auto ">
// //             {/* <KmlFileUploding/> */}
// //             <KmZFileUploding map={mapRef.current}/>
// //             <div className="sticky top-52 right-7 z-50">
// //               <label
// //                 htmlFor="kml-upload"
// //                 className="fixed top-52 right-8 flex justify-center items-center gap-2 bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-4 cursor-pointer py-3 rounded-lg shadow-lg z-50"
// //               >
// //                 <Upload className="w-5 h-5" />
// //                 Upload KML
// //               </label>
...
// //                     <Polygon key={idx} positions={polygon} color="blue" />
// //                   ))}
