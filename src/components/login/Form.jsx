...
import { authAPI } from "@/utils/auth"; // Import the auth utility

export default function Form() {
  const [username, setUsername] = useState(""); // Changed email to username to match backend usually, but UI might want Email.
  // Actually, standard Django AbstractUser uses 'username'. 
  // If the user wants to login with Email, we need to handle that.
  // For now let's assume the user enters their username in the "Email" field or we treat email as username.
  // The prompt said "email names", and the form field is "email".
  // I will send the value as 'username' to the backend to match standard TokenObtainPairView.
  ...
  const handleSubmit = async (e) => {
    ...
    setApiError("");
    const newErrors = {};
    if (!username.trim()) newErrors.username = "Username/Email is required";
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        ...
        // Save tokens to localStorage for legacy/external API support (like EarthScan)
        if (data && data.access) {
        ...
        // We intentionally do NOT set loading(false) here to keep the loader visible during redirect.
      } catch (err) {
        console.error("Login failed", err);
        const errorMsg = err.response?.data?.detail || "Invalid credentials. Please try again.";
        setApiError(errorMsg);
        alert(errorMsg); // Show alert as requested
            ...
            <div className="w-full lg:w-1/2 h-full flex items-center justify-center shadow-2xl p-8 lg:p-12">
              <div className="w-full max-w-md">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div>
                    <div
                      className={`flex items-center gap-3 border-2 bg-white px-4 py-3 rounded-xl shadow-sm transition-all ${errors.username
                        ? "border-red-500"
                        : username
                          ? "border-green-500"
                          : "border-gray-200"
                    ...
                    >
                      <User className={`h-6 w-6 ${errors.username ? "text-red-500" : username ? "text-green-500" : "text-gray-400"
                        }`} />
                      <input
                      ...
                      {username && (
                        <div className="flex-shrink-0">
                          {errors.username ? <AlertTriangle className="h-5 w-5 text-red-500" /> : <CheckCircle className="h-5 w-5 text-green-500" />}
                        </div>
                      )}
                    </div>
                    {errors.username && (
                      <p className="text-red-500 text-sm mt-1 flex items-center gap-1">
                        <AlertTriangle className="h-4 w-4" /> {errors.username}
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    <div
                      className={`flex items-center gap-3 border-2 bg-white px-4 py-3 rounded-xl shadow-sm transition-all ${errors.password
                        ? "border-red-500"
                        : password
                          ? "border-green-500"
                          : "border-gray-200"
                    ...
                    >
                      <Lock className={`h-6 w-6 ${errors.password ? "text-red-500" : password ? "text-green-500" : "text-gray-400"
                        }`} />
                      <input
                        className="w-full p-2 outline-none bg-transparent text-gray-700 placeholder-gray-400 text-lg"
